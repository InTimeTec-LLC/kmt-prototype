...
@Slf4j
@Configuration
public class AttachmentTestDataRepository {

    private static HashMap<String, Attachment> attachments;

    public AttachmentTestDataRepository() {
    }

    public synchronized HashMap<String, Attachment> getAttachments() {

        if (attachments == null) {
            attachments = getTestDataMap(Attachment.class, "AttachmentTestData.json");
        }
        return attachments;
    }
